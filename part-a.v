...
From ExtLib Require Import Monad OptionMonad ListMonad.
Import QcNotation MonadNotation ListNotations.
...
Inductive env : Set :=
| empty : env
| evar : env -> typ -> env    (* type of var *)
| ebound : env -> typ -> env  (* type of tvar *)
.
...
Fixpoint measure_typ (T : typ) : nat :=
  match T with
  | tvar _ => 1
  | top => 1
  | arrow T1 T2 => measure_typ T1 + measure_typ T2 + 1 
  | all T1 T2 => measure_typ T1 + measure_typ T2 + 1 
  end. 
...
Fixpoint get_typ (e : env) (t : term) : option typ :=
  ...
  | tapp t1 T2 => 
      match get_typ e t1 with
      | Some (all T11 T12) =>
          match sub_check e T2 T11 (measure_typ T2 + measure_typ T11) with
...
Definition type_check (e : env) (t : term) (T : typ) : bool :=
  ...
  | Some T' =>
      match sub_check e T' T (measure_typ T' + measure_typ T) with
      | Some true => true
      | _ => false
      end
  | None => false
...
Fixpoint ctx_app (c : ctx) (t : term) {struct c} : term :=
  ...
  | @c_apparg t' H c' => app t' (ctx_app c' t)
...
Program Fixpoint step (t : term) {measure (measure_term t) lt} : option term :=
  match t with
  | app (abs _ t12) t2 =>
...
Qed.
Next Obligation.
  apply (measure_term_le_ctx_inj Heq_anonymous).
Qed.
Solve All Obligations with easy.
...
Ltac tvar_case :=
  ...
  | |- ?x =>
      ...
      | context C [(lt_eq_lt_dec ?n ?n')] =>
          case (lt_eq_lt_dec n n'); [intros H; case H; clear H | idtac ]
      end
  end.
...
Ltac common_cases n T :=
  ...
  [ repeat tvar_case;
    simpl; trivial; try (intros; apply f_equal with (f := tvar); lia);
    try (intros; assert False; [ lia | contradiction ])
  | trivial
...
Lemma get_bound_insert_bound_ge :
  ...
  simpl; intros n n' e e' H; generalize n; clear n; induction H; simpl; trivial;
    ...
    [ inversion H1
    | clear IHn'; rewrite IHinsert_bound; try lia;
...
Definition gen_typ_from_env (E : env) : G (option typ) :=
  match count_var E with
  | O => ret None
  | S n => n' <- choose(0, n);; ret (get_var E n')
  end.
...
Lemma get_bound_narrow_ne :
  ...
  intros n n' e e' H; generalize n'; clear n'; induction H; intros n' H1; simpl;
    ...
    | induction n'; trivial; rewrite IHnarrow; trivial; lia
    | auto ].
Qed.
...
Lemma transitivity_case :
  ...
  intros Q H e S T H1 H2; induction H1;
    ...
    | inversion_clear H2;
      ...
      | apply SA_Arrow;
        [ assert (H5 : size T1 < size (arrow T1 T2)); try (simpl; lia);
          generalize (H _ H5); intros (H6, _); exact (H6 _ _ _ H0 H1_)
        | assert (H5 : size T2 < size (arrow T1 T2)); try (simpl; lia);
    ...
    | inversion_clear H2;
      [ apply SA_Top; trivial;
        ...
        | apply wf_typ_ebound with (U := T1);
          apply (proj1 (proj2 (sub_wf H1_0))) ]
      | assert (H5 : size T1 < size (all T1 T2)); try (simpl; lia);
        apply SA_All;
        [ generalize (H _ H5); intros (H6, _); exact (H6 _ _ _ H0 H1_)
...
Lemma transitivity_and_narrowing :
  ...
  assert
    ...
    [ induction n;
      [ intros Q H; assert False; [ lia | contradiction ]
      | intros Q H; case (le_lt_or_eq _ _ H);
        [ intro H'; apply IHn; lia
...
Lemma env_subst_get_bound_ge :
  ...
  intros n n' e e' T H; generalize n; clear n;
    ...
    | induction n';
      [ inversion H1
      | clear IHn'; replace ((S n') - 1) with (S (n' - 1)); try lia;
        rewrite IHenv_subst; try lia;
        case (get_bound e n'); simpl; trivial; intro T'';
        apply f_equal with (f := @Some typ);
        apply (tshift_tsubst_prop_1 0 X) ] ].
Qed.
  ...
  intros e n t U H1 H2; cut (exists e', e' = remove_var e n);
    [ intros (e', E); rewrite <- E in H2; generalize n e E H1; clear n e E H1;
      ...
      [ apply T_Var; trivial; rewrite E in H0;
        ...
        [ rewrite get_var_remove_var_ge in H0; trivial; lia
        | rewrite get_var_remove_var_lt in H0; trivial; lia ]
      ...
      | intro H2; apply T_Var;
        ...
        | induction x;
          ...
          | simpl; rewrite <- minus_n_O;
            rewrite get_var_remove_var_ge; trivial; lia ] ] ]
...
Lemma tsubst_preserves_subtyping :
  ...
  intros e e' n T U V H1 H2; generalize n e' T H1; clear n e' T H1; induction H2;
    ...
    | intros n e' T' H1; simpl; case (lt_eq_lt_dec X n);
      ...
      | intro H5; apply SA_Trans_TVar with (U := tsubst U n T');
        [ rewrite env_subst_get_bound_ge with (1 := H1); try lia;
          induction X;
