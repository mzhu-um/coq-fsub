...
experiments :: IO ()
experiments = do
  let mutants = hardMutants
  forM_ mutants $ \m -> do
    putStrLn $ show m
    let ?mutant = m 
    quickCheckWith stdArgs{maxSuccess=100000} $ forAll genExpr $ propTimeout 10000 prop_ppreservation
--  let t = 100
